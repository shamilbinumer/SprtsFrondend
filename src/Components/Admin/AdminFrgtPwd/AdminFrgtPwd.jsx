...
const EditPwd = async (e) => {
  e.preventDefault();
  try {
    const emailResponse = await axios.get(`http://localhost:7000/sportstrack/getemail/${val.email}`);
    ...
    if (data.email !== val.email) {
      ...
      const passwordResponse = await axios.patch("http://localhost:7000/sportstrack/adminpassword", {
